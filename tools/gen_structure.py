...
def build_cards(spr, meta):
    ...
    for p in spr['сотрудники']:
        # Вакансии в состав не выводятся: показывать пустые строки незачем.
        if p['фио'].lower().startswith('вакан'):
            continue
...
def card_html(c):
    ...
    if рядовые:
        # Сотрудники сгруппированы по отделам и свёрнуты: иначе карточка
        # крупного управления разворачивается в длинную простыню.
        по_отделам = OrderedDict()
        for p in рядовые:
            по_отделам.setdefault(p['отдел'] or 'Без отдела', []).append(p)
        блоки += '<h5 class="vz-h5">Сотрудники</h5>'
        for отдел, люди in по_отделам.items():
            блоки += (
                '<details class="vz-dept"><summary>%s<span>%d %s</span></summary>'
                '<div class="vz-dept-body">%s</div></details>' % (
                    esc(отдел), len(люди),
                    plural(len(люди), 'человек', 'человека', 'человек'),
                    ''.join(person_html(p) for p in люди)))
    if not блоки:
    ...
    return (
        ...
        '          </div>\n'
        '          <div class="vz-people" hidden>%s</div>\n'
        '        </article>\n' % (
