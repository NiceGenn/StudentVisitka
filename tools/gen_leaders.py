...
def build_cards(spr, meta):
    ...
    по_фио = {(p['подразделение'], p['фио']): p for p in spr['руководители']}
    for c in cards:
...
def card_html(c):
    ...
    if not блоки:
        блоки = '<p class="vz-none">В справочнике руководитель не указан</p>'
    ...
    return (
        ...
        '            <div class="vz-card-title">\n'
        '              <h4>%s</h4>\n'
        '              <div class="vz-metaline">%s</div>\n'
        '            </div>\n'
        '          </div>\n'
        '          <div class="vz-func">\n'
        '            <h5 class="vz-h5">Чем занимается</h5>\n'
        '            <p>%s</p>\n'
        '            <ul class="vz-tags">%s</ul>\n'
        '          </div>\n'
        '          <div class="vz-people">%s</div>\n'
        '        </article>\n' % (
...
def main():
    ...
    страница = шапка.replace('{{СТИЛИ}}', стили).replace('{{СОДЕРЖИМОЕ}}', (
        ...
        'Благовещенского муниципального округа — кто за что отвечает и как связаться</p>\n'
        '\n'
        ...
        '        </div>\n'
        '        <p class="vz-status" id="vz-status">%d %s в %d %s</p>\n'
        '%s'
        '        <p class="vz-empty" id="vz-empty" hidden>Ничего не найдено. '
        'Попробуйте другой запрос или снимите фильтр по разделу.</p>\n'
        '        <p class="vz-note">Сведения о руководителях приведены по телефонному справочнику '
        ...
        'о подразделениях и подлежат уточнению по действующим положениям.</p>\n' % (
            ...
            всего_людей, plural(всего_людей, 'руководитель', 'руководителя', 'руководителей'),
            len(cards), plural(len(cards), 'подразделении', 'подразделениях', 'подразделениях'),
            секции, esc(spr['актуально'].rstrip('.')))))

    OUT.write_text(страница + подвал, encoding='utf-8')
    print('Карточек:      %d' % len(cards))
    print('Руководителей: %d' % всего_людей)
